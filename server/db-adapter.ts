/**
 * Adaptateur de base de données qui gère les erreurs de better-sqlite3
 * et fournit une implémentation alternative en mémoire si nécessaire.
 */
...
class MemoryStore {
  ...
  constructor() {
    ...
    for (const key in schema) {
      if (typeof schema[key] === 'object' && schema[key]._.name && schema[key]._.name.name) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // Essayer d'importer de manière dynamique
        Database = await import('better-sqlite3').then(m => m.default).catch(e => null);
      ...
      try {
        ...
        console.error(`Erreur de permission sur le répertoire ${dbDir}:`, permError);
        // Essayer de corriger les permissions
        try {
          fs.chmodSync(dbDir, 0o777);
          console.log(`Permissions du répertoire ${dbDir} mises à jour`);
        } catch (chmodError) {
          console.error(`Impossible de modifier les permissions du répertoire ${dbDir}:`, chmodError);
          throw new Error('Problème de permissions sur le répertoire de données');
        }
      }
      ...
      try {
        ...
        // Essayer avec une base de données en mémoire SQLite
        try {
          console.log('Tentative d\'utilisation d\'une base de données SQLite en mémoire...');
          sqlite = new Database(':memory:', { verbose: console.log });
        } catch (memDbError) {
          console.error('Erreur lors de la création de la base de données en mémoire SQLite:', memDbError);
          throw new Error('Impossible de créer une base de données SQLite');
        }
      }
